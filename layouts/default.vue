...
export default defineNuxtComponent({
    ...
    computed: {
        page() {
            if (!this.pagination.previous_page_url && !this.pagination.next_page_url) return `/?page=1`
    ...
    methods: {
        async fetchData() {
            try {
                ...
                const { data, error } = await useFetchDrinks(`/drinks${this.page}&rowsPerPage=25`, { query })
